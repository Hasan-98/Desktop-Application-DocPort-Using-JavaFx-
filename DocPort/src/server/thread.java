...
public class thread extends Thread {
	...
	thread(Socket Socket1, String phone, Vector connectedSockets, Vector connectedClints, Message message, User user,
			Friend friend, DataOutputStream outToClient, Vector<Vector<String>> Groups) {
				...
				try {
					if (method.equals("EXIT")) {
						...
						synchronized (this) {
							...
							int id2 = user.getUserId(phone);
							// System.out.println(id1);
							// System.out.println(id2);
							if (id1 > 0 && id2 > 0 && id1 != id2) {
								boolean inserted = friend.insert(id1, id2);
								if (inserted)
						...
						synchronized (this) {
							...
							String count = "";
							count = inFromClient.readLine();// count =
						...
						synchronized (this) {
							...
							System.out.println("File " + fileName + " received from " + phone);
							...
							for (int i = 0; i < connectedClients.size(); i++) {
								if (connectedClients.get(i).equals(to)) {
									...
									// System.out.println("file read by
									// server");
									...
									// Sending file name and file size to the
									// server
									...
									DataOutputStream outToClient1 = new DataOutputStream(
											connectedSockets.get(i).getOutputStream());
									// if (request == 0)
									{
										outToClient1.writeBytes("VIDEO" + "\n");
										// request=1;
									}
									outToClient1.writeBytes(phone + "\n");
						...
						synchronized (this) {
							...
							try {
								onlineFriends = user.getFriend(phone);
							...
							for (int i = 0; i < Groups.size(); ++i) {
								if (Groups.get(i).contains(phone)) {
									++noOfGroups;
								}
								/*
								 * for(int j = 0; j < Groups.get(i).size(); ++j)
								 * { System.out.println("Memeber names: " +
								 * Groups.get(i).get(j)); }
								 */
							}
							// send Groups
							...
							try {
								offlineFriends = user.getFriendOffline(phone);
							} catch (SQLException ex) {
								Logger.getLogger(thread.class.getName()).log(Level.SEVERE, null, ex);
							}
					...
					/*
					 * try { connectionSocket.close(); } catch (IOException ex)
					 * { Logger.getLogger(thread.class.getName()).log(Level.
					 * SEVERE, null, ex); }
					 */
				}
			}
